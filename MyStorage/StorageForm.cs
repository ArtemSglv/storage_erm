    ...
    public partial class StorageForm : Form
    {
        ...
        private void GetRemains()
        {
            string sql = "select lp.prod_id as 'ID', pos.name as 'Shop', pr.name as 'Prodact name', lp.count as 'Count', pr.description as 'Description', lp.price as 'Price'" +
                         "from listproducts lp join products pr on lp.prod_id = pr.id join placesofsale pos on pos.id = lp.place_id where pos.type_id = (select id from typeplace where type = 'storage'); ";
        ...
        {
            string sql = "select id,name from placesofsale where type_id=(select id from typeplace where type='storage');";
